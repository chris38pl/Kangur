...
function readVersionEnv(name: string): string {
  const raw = process.env[name]?.trim();
  return raw && raw.length > 0 ? raw : DEFAULT_VERSION;
}
...
export function getAppVersion(): AppVersionResponseDto {
  return {
    latestVersion: readVersionEnv("APP_LATEST_VERSION"),
    minSupportedVersion: readVersionEnv("APP_MIN_SUPPORTED_VERSION"),
