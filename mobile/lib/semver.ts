...
/** true when `latest` is greater than `installed` (plain MAJOR.MINOR.PATCH). */
export function isVersionNewer(latest: string, installed: string): boolean {
  return compareSemver(latest, installed) === 1;
}

/** true when `current` is greater than `lastSeen` (plain MAJOR.MINOR.PATCH). */
export function isNewerVersion(current: string, lastSeen: string): boolean {
  return compareSemver(current, lastSeen) === 1;
}
